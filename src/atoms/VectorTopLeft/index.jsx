import React from 'react'

const VectorTopLeft = (props) => {
    return (
        <svg {...props} width="243" height="565" viewBox="0 0 243 565" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M1.5 3C104.5 41.1667 293.4 206.8 225 564" stroke="url(#paint0_linear_16_174)" stroke-opacity="0.06" stroke-width="6" />
            <defs>
                <linearGradient id="paint0_linear_16_174" x1="-23.5" y1="-37.5" x2="333" y2="602.5" gradientUnits="userSpaceOnUse">
                    <stop stop-color="#343045" />
                    <stop offset="0.151042" stop-color="#C0B7E8" />
                    <stop offset="0.760417" stop-color="#8176AF" />
                    <stop offset="0.973958" stop-color="#343045" />
                </linearGradient>
            </defs>
        </svg>

    )
}

export default VectorTopLeft
